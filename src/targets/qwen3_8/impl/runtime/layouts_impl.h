...
#include "targets/qwen3_8/impl/runtime/workspace_recipe.h"
...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    out.decoder = qwen3_8::plan_decoder_state(
        builder, qwen3_8::DecoderStateSpec{
    ...
    if constexpr (Variant::supports_dflash) {
        if (plan.features.dflash()) {
            ...
            dflash.full = qwen3_8::PagedKVCacheLayout{
    ...
    out.round = qwen3_8::begin_round_state_layout(
        builder, qwen3_8::RoundStateSpec{.hidden         = TextConfig::hidden,
    ...
    qwen3_8::complete_round_state_layout(builder, out.round);
...
WorkspacePlan build_workspace_plan(const SequencePlanImpl& plan) {
    ...
    target_body(text_prefill, 1, chunk, qwen3_8::TextPhase::Prefill, GdnWorkspacePath::Prefill, 1,
    ...
    for (std::int32_t batch = 1; batch <= static_cast<std::int32_t>(plan.max_concurrency);
        ...
        target_body(ordinary, batch, batch, qwen3_8::TextPhase::Verify, GdnWorkspacePath::Snapshot,
    ...
    if (plan.features.mtp()) {
        ...
        target_body(mtp_prefill, 1, chunk, qwen3_8::TextPhase::Prefill, GdnWorkspacePath::Prefill,
        ...
        for (std::int32_t batch = 1; batch <= static_cast<std::int32_t>(plan.max_concurrency);
            ...
            target_body(target, aggregate, aggregate, qwen3_8::TextPhase::Verify,
...
std::unique_ptr<qwen3_8::detail::SequencePlannerImpl<Variant>>
make_sequence_planner_impl(DeviceContext& device, const EngineOptions& options,
    ...
    auto planner     = std::make_unique<qwen3_8::detail::SequencePlannerImpl<Variant>>();
