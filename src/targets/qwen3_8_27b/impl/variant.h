...
using GraphExecutionProfile = qwen3_8::GraphExecutionProfile;

// Compile-time data and the three closed execution leaves supplied to the Qwen3.8 family runtime.
...
struct Variant {
    ...
    using VisionWeights                  = qwen3_8::VisionWeights;
    ...
    gdn_input_projection_snapshot(const Tensor& hidden, const GdnProjectionWeights& weights,
                                  ...
                                  Tensor& value, Tensor& output_gate, qwen3_8::TextPhase phase,
                                  WorkspaceArena& workspace, cudaStream_t stream);
    ...
    [[nodiscard]] static std::size_t gdn_input_projection_snapshot_workspace_capacity_bytes(
        WeightsProfile weights_profile, qwen3_8::TextPhase phase, std::int32_t batch_size,
        std::int32_t first, std::int32_t last);
